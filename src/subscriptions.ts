...
exports.mirrorSubscriptionsFollowed = (async (snapshot, context) => {
      ...
      try {
        ...
        const follower_info = await admin.database().ref("users/" + follower_id).once("value");
        const follower_username = follower_info.val().username as string;
        ...
        const tokens_snap = await admin.database().ref(`users/${ followed_id }/fcm_tokens`).once("value");
